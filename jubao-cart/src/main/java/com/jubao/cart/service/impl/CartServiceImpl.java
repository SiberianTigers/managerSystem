...
public class CartServiceImpl implements CartService {
	...
	public JubaoResult addItem(long pid, String itemInfo, int num, HttpServletRequest request,
		...
		try {
			...
			if (jb.getStatus() == 200) {
				cart.setCount(cart.getItemList().size());// 设置购物车中的商品项数量
	...
	public void setCartRedis(Cart cart, HttpServletRequest request) {
		...
		if (!StringUtils.isBlank(token)) {
			...
			for (CartItem cartItem : redisCart.getItemList()) {

				for (CartItem cartItem2 : cart.getItemList()) {

					if (!(cartItem.getItem().getId().longValue() == cartItem2.getItem().getId().longValue()
							&& cartItem.getItemType().equals(cartItem2.getItemType()))) {
						redisCart.getItemList().add(cartItem2);
						flag = true;
					}
				}
